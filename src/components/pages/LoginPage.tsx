...
export default function LoginPage() {
  const { login, logout, user, navigateTo } = useStore();
  const [tab, setTab]           = useState<"patron" | "admin">("patron");
  const [patronName, setPatronName] = useState("Aishwarya Iyer");
  const [patronEmail, setPatronEmail] = useState("aishwarya@patron.in");
  const [adminEmail, setAdminEmail] = useState("admin@hamsinisilks.com");
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gold-100 overflow-hidden">
          ...
          <div className="bg-gradient-to-br from-maroon-900 to-maroon-800 p-8 text-center relative overflow-hidden flex flex-col items-center">
            ...
            <div className="relative z-10">
              <h1 className="font-display text-2xl font-bold text-gold-200">Hamsini Patron Portal</h1>
              <p className="text-xs text-gold-100/70 mt-1">Access your orders, wishlist & exclusive privileges</p>
            </div>
          </div>
          ...
          <div className="p-6">
            {user.loggedIn ? (
              ...
              <div className="text-center space-y-4">
                ...
                <div>
                  <h3 className="font-display text-lg font-bold text-maroon-900">{user.name}</h3>
                  ...
                  <span className="inline-block mt-1.5 text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-maroon-100 text-maroon-800">
                    {user.role === "admin" ? "⚙️ Admin Access" : "👑 Privileged Patron"}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <button
                    onClick={() => user.role === "admin" ? navigateTo("admin") : navigateTo("shop")}
              ...
              <div>
                ...
                <div className="flex bg-maroon-50 rounded-xl p-1 mb-5">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-bold tracking-wide transition-all cursor-pointer ${
                      tab === "patron" ? "bg-white shadow text-maroon-900" : "text-maroon-600 hover:text-maroon-800"
                    }`}
                  >
                    👑 Patron Login
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-bold tracking-wide transition-all cursor-pointer ${
                      tab === "admin" ? "bg-white shadow text-maroon-900" : "text-maroon-600 hover:text-maroon-800"
                    }`}
                  >
                    ⚙️ Admin Login
                  </button>
                ...
                {tab === "patron" ? (
                  <form
                    onSubmit={(e) => { e.preventDefault(); login(patronEmail, "customer", patronName); }}
                    className="space-y-4"
                  >
                    <div>
                      <label className="block text-xs font-bold text-maroon-900 mb-1.5">Your Name</label>
                      <input
                        type="text"
                        required
                        value={patronName}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-maroon-900 mb-1.5">Email Address</label>
                      <input
                        type="email"
                        required
                        value={patronEmail}
                    ...
                    <div className="bg-gold-50 rounded-xl p-3 text-xs text-maroon-800 border border-gold-200">
                      💡 <strong>Demo mode:</strong> Any email works. No real authentication required.
                    </div>
                    <button
                      type="submit"
                  ...
                  <form
                    onSubmit={(e) => { e.preventDefault(); login(adminEmail, "admin", "Hamsini Admin"); }}
                    className="space-y-4"
                  >
                    <div>
                      <label className="block text-xs font-bold text-maroon-900 mb-1.5">Admin Email</label>
                      <input
                        type="email"
                        required
                        value={adminEmail}
                    ...
                    <div className="bg-maroon-50 rounded-xl p-3 text-xs text-maroon-800 border border-gold-100">
                      🔐 <strong>Admin access</strong> unlocks full dashboard — orders, catalogue & media management.
                    </div>
                    <button
                      type="submit"
                ...
                <div className="text-center mt-4">
                  <button onClick={() => navigateTo("home")} className="text-xs text-maroon-600 hover:text-maroon-800 cursor-pointer underline">
