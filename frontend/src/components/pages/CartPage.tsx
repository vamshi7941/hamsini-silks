      ...
      <div className="bg-white border-b border-gold-100 px-4 sm:px-8 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <h1 className="font-display text-xl sm:text-2xl font-bold text-maroon-900">
            Shopping Bag <span className="text-maroon-400 text-sm font-normal">({cart.length} {cart.length === 1 ? "item" : "items"})</span>
          </h1>
          <button onClick={() => navigateTo("shop")} className="text-sm text-gold-600 font-bold hover:text-gold-700 cursor-pointer">
            ← Continue shopping
          </button>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {cart.length === 0 ? (
          ...
          <div className="text-center py-24 max-w-md mx-auto">
            ...
            <h2 className="font-display text-2xl font-bold text-maroon-900 mb-2">Your bag is empty</h2>
            <p className="text-sm text-maroon-700/70 mb-6">Discover our heirloom silk sarees and add the ones that speak to your heart.</p>
            <button
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-7 xl:col-span-8 space-y-4">
                ...
                return (
                  <div key={p._id} className="bg-white rounded-2xl border border-gold-100 shadow-xs p-4 flex gap-4 hover:border-gold-300 transition-colors">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                      <div>
                        <span className="text-[10px] font-bold text-gold-600 uppercase tracking-wider">{p.category}</span>
                        <h3
                        ...
                        <div className="flex items-baseline gap-2 mt-1">
                          <span className="font-bold text-maroon-900">₹{p.price.toLocaleString("en-IN")}</span>
                          {p.originalPrice && <span className="text-xs text-maroon-400 line-through">₹{p.originalPrice.toLocaleString("en-IN")}</span>}
                          {d > 0 && <span className="text-xs text-emerald-700 font-bold">{d}% off</span>}
                        </div>
                      </div>
              ...
              {/* Shipping indicator */}
              <div className={`rounded-2xl p-4 border-2 text-sm font-medium ${shippingFree ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-gold-50 border-gold-200 text-gold-800"}`}>
                {shippingFree
            ...
            <div className="lg:col-span-5 xl:col-span-4">
              <div className="bg-white rounded-2xl border border-gold-100 shadow-xs p-5 sticky top-24 space-y-5">
                <h2 className="font-display text-base font-bold text-maroon-900 border-b border-gold-100 pb-3">Order Summary</h2>

                {/* Price breakdown */}
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    <span className="text-maroon-700">Subtotal ({cart.reduce((a, b) => a + b.quantity, 0)} items)</span>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={shippingFree ? "text-emerald-700 font-semibold" : "font-semibold text-maroon-900"}>
                  ...
                  <div className="flex justify-between font-bold text-base border-t border-gold-100 pt-3">
                    ...
                    <span className="font-display text-maroon-900 text-lg">₹{(finalTotal + (shippingFree ? 0 : 150)).toLocaleString("en-IN")}</span>
                  </div>
                </div>

                {/* Coupon */}
                <form onSubmit={handleCoupon} className="space-y-2">
                  <label className="text-xs font-bold text-maroon-900 uppercase tracking-wider block">Voucher Code</label>
                  ...
                  {discount === 0 && <p className="text-[10px] text-maroon-600/70">💡 Try <strong>BRIDE30</strong> for 30% bridal discount</p>}
                </form>
                ...
                <div className="grid grid-cols-3 gap-2 pt-1">
                  {[
                    ...
                    <div key={t.label} className="text-center bg-maroon-50 rounded-xl p-2">
                      <div className="text-lg">{t.icon}</div>
                      <div className="text-[10px] font-semibold text-maroon-800 mt-0.5">{t.label}</div>
                    </div>
                  ))}
                </div>
              </div>
